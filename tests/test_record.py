...
class TestIntegrations(BaseTestCase):
    ...
    def test_returns_all_recordtitle(self):
        self.register_user()
        login = self.login_user()
        resp = json.loads(login.data.decode("UTF-8"))
        token = resp['token']
    ...
    def test_add_empty_record_category(self):
        ...
        token = resp['token']
        response = self.app.post('/api/v1/record'),
        data =json.dumps(dict(recordname="bad roads", description="yoyo", category="", location="NYS")),
        headers={"Authorization":"Bearer {}".format(token)},content_type="application/json")
        response_msg = json.loads(response.data.decode("UTF-8"))
    ...
    def test_record_can_be_edited(self):
